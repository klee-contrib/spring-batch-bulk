...
public class JdbcEntityRowMapper<T> implements RowMapper<T> {
	...
	public JdbcEntityRowMapper(Class<T> mappedClass) {
	...
	/**
	 * Checks <code>@Column</code> annotation with name attribute on getter.
	 * 
	 * @param mappedClass - initialized class of bean
	 * @param pd          - descriptor of initialized property
	 */
	private void mapAnnotated(Class<?> clazz, Field field) {

		var column = field.getAnnotation(Column.class);

		if ((column != null) && StringUtils.hasText(column.value())) {
			mappedFields.put(column.value(), BeanUtils.getPropertyDescriptor(clazz, field.getName()));
	 ...
	 * primitives.
	 * <p>
	 * If this flag is set to {@code true} and you use an <em>ignored</em> primitive
	 * property value from the mapped bean to update the database, the value in the
	 * database will be changed from {@code NULL} to the current value of that
	 * primitive property. That value may be the property's initial value
	 * (potentially Java's default value for the respective primitive type), or it
	 * may be some other value set for the property in the default constructor (or
	 * initialization block) or as a side effect of setting some other property in
	 * the mapped bean.
	 */
	...
	public T mapRow(ResultSet rs, int rowNumber) throws SQLException {
		...
		for (int index = 1; index <= columnCount; index++) {
			...
			PropertyDescriptor pd = (this.mappedProperties != null ? this.mappedFields.get(property) : null);
			if (pd != null) {
				try {
	 ...
	 * 
	 * @param mappedClass       the class that each row should be mapped to
	 * @param conversionService the {@link ConversionService} for binding JDBC
	 *                          values to bean properties, or {@code null} for none
	 * @since 5.2.3
